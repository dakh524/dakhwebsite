            ...
            {navLinks.map((link) => (
              link.dropdown ? (
                <div key={link.name} className="relative group">
                  <button className={`text-[11px] font-black tracking-widest uppercase transition-all py-1 flex items-center gap-1 ${
                    link.dropdown.some(item => location.pathname === item.path) ? 'text-primary' : 'text-slate-400 hover:text-white group-hover:text-white'
                  }`}>
                    {link.name}
              ...
              >
                {link.dropdown ? (
                  <div className="py-2">
                    ...
                    >
                      {link.name}
                      <ChevronDown size={28} className={`transition-transform duration-300 ${mobileDropdownOpen === link.name ? 'rotate-180 text-primary' : ''}`} />
                    </button>
